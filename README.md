...
|------|-----------|-----------|-------------|
| Infrastructure | NVDA | 2.957 | Sells the GPUs |
| AI Builders | META, GOOGL | 1.832 | Proprietary AI (Llama, Gemini, custom chips) |
| AI Integrators | MSFT, AMZN | 0.942 | Third-party partnerships (OpenAI, Anthropic) |
...
**Finding:** [FILL IN ON APRIL 1 — e.g. "The builder premium is strongest in [X] regimes (+Y%) and weakest in [Z] regimes (+W%), suggesting the advantage is [macro-sensitive / macro-resilient]."]

| Regime | Avg Builder Premium | Months |
|--------|-------------------|--------|
| [TOP REGIME — from regime_summary.csv] | [+X%] | [N] |
| [2ND REGIME] | [+X%] | [N] |
| [WEAKEST REGIME] | [+X%] | [N] |

Charts generated by `make regime_charts` — see `stock_pipeline/regime_charts/` for heatmap, bar chart, time series, and decision matrix. Methodology: `stock_pipeline/macro_regime_analysis.py`. Raw data: `make regime` → `regime_analysis.csv` + `regime_summary.csv`.

![Dashboard](dashboard.png)

In 2026, Big Tech will spend ~$650B on AI infrastructure. But spending more doesn't mean earning more — Meta spends the least of the four hybrids ($125B) yet delivers the highest Sharpe ratio (1.998) because nearly 100% of its capex goes to proprietary AI. Amazon spends the most ($200B) but dilutes returns across logistics and third-party partnerships.
